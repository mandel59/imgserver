...
- When committing, use the `jj commit fileset... -m "title" -m "description" -m "Related: #123" -m "Generated-by: Codex/GPT-5"` form, with the relevant issue number.  
  Example:

  ```sh
  jj commit path/to/file.ts -m "Refactor DB executor boundary" -m "Introduce SqlExecutor and move sql.js-specific statement handling into an adapter." -m "Related: #5" -m "Generated-by: Codex/GPT-5"
  ```
